            ...
            <form id="eventForm" action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card card-success my-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <textarea class="form-control" name="shortDescription" id="shortDescription" maxlength="100"
                                    placeholder="Short Description (max 100 characters)"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <select id="city" name="city" class="form-control">
                                    <option value="" selected>Event City</option>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                ...
                                <select id="ageRestrictions" name="age_restrictions" class="form-control">
                                    <option value="" selected>Age Restrictions</option>
                                    <option>None</option>
    ...
    <script>
        ...
        function validateForm() {
            ...
            if (category === '-- Select --' || title.trim() === '' || startDate.trim() === '' ||
                ...
                bookingDeadline.trim() === '' || $('#shortDescription').summernote('isEmpty')) {
            ...
            if (ageRestrictions === 'Minimum Age') {
                if (minAge.trim() === '') {
                    alert('Please fill out the Minimum Age field in Step 2.');
                    isValid = false;
                    return isValid;
                }
            ...
            if (ageRestrictions === 'Maximum Age') {
                if (maxAge.trim() === '') {
                    alert('Please fill out the Maximum Age field in Step 2.');
                    isValid = false;
                    return isValid;
                }
            ...
            ticketSections.each(function(index, section) {
                ...
                var ticketDescription = $(section).find('.form-group input[name="ticket_description[]"]').val();
                var ticketPrice = $(section).find('.form-group input[name="ticket_price[]"]').val();
                var ticketQuantity = $(section).find('.form-group input[name="ticket_quantity[]"]').val();
                ...
                if (parseFloat(ticketPrice) < 0) {
                    isValid = false;
                    alert('Ticket price cannot be negative in Step 3.');
                    return isValid;
                }

                if (parseFloat(ticketQuantity) < 0) {
                    isValid = false;
                    alert('Number of Tickets cannot be negative in Step 3.');
                    return isValid;
                }
            });


            // Step 4 Validation (Group Tickets)
            var group_ticketName = $('#group_ticket_name').val();
            var group_ticketDescription = $('#group_ticket_description').val();
            var group_ticketPrice = $('#group_ticket_price').val();
            var group_ticketQuantity = $('#group_ticket_quantity').val();
            ...
            // Check if any of the fields is filled
            if (group_ticketName.trim() !== '' || group_ticketDescription.trim() !== '' ||
                group_ticketPrice.trim() !== '' || group_ticketQuantity.trim() !== '' || group_count.trim() !== '') {
                // Validate if all fields are filled
                if (group_ticketName.trim() === '' || group_ticketDescription.trim() === '' ||
                    group_ticketPrice.trim() === '' || group_ticketQuantity.trim() === '' || group_count.trim() === '') {
                    alert('Please fill out all group ticket fields in Step 4.');
                    isValid = false;
                    return isValid;
                }
                // Validate that price is not negative
            ...
            // Else part of form submission validation
            var additionalInfo = $('#additionalInfo').val();
            if (additionalInfo.trim() === '') {
                alert('Please fill out all required fields.');
                isValid = false;
